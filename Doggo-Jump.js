...
Brick.prototype.draw = function() {
    var accel = new PVector(this.accel.x, this.accel.y);
    accel.add(Gravity);
...
Ground.prototype.draw = function() {
    ...
    for(var j = this.x; j <= width; j += 90) {
        image(getImage("cute/GrassBlock"), j, this.y - 50);
...
Enemy.prototype.hitMyHead = function(player) {
    var head = {x: this.x-35, y: this.y - 93, width: 26, height: 25};
    return collides(player, head); 
};

Enemy.prototype.hitMyBody = function(player) {
    var body = {x: this.x-58, y: this.y - 65, width: 62, height: 70};
